...
async def test_get_shop_items_pagination(
    client: AsyncClient, create_test_item_for_shop: Item
):
...
async def test_get_shop_item_by_id(
    ...
    response = await client.get(f"/api/v1/shop_items/{shop_item_id}")
    assert response.status_code == 200
    response_data = response.json()
    ...
    response_not_found = await client.get("/api/v1/shop_items/99999")
...
async def test_delete_shop_item(
    ...
    response = await client.delete(f"/api/v1/shop_items/{shop_item_id}")
    assert response.status_code == 204

    deleted_shop_item = await db_session.get(ShopItem, shop_item_id)
    assert deleted_shop_item is None

    response_not_found = await client.delete("/api/v1/shop_items/99999")
    assert response_not_found.status_code == 404
    assert "detail" in response_not_found.json()
...
async def test_get_shop_item_count(
    ...
    response_initial = await client.get("/api/v1/shop_items/count")
...
async def test_check_shop_item_exists(
    ...
    response_exists = await client.get(f"/api/v1/shop_items/{shop_item_id}/exists")
    assert response_exists.status_code == 200
    assert response_exists.json()["exists"] is True

    response_not_exists = await client.get("/api/v1/shop_items/99999/exists")
