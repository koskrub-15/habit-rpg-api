...
async def test_get_notifications_filter_by_name(
    ...
    response = await auth_client.get("/api/v1/notifications/?name=Welcome")
    assert response.status_code == 200
    response_data = response.json()
...
async def test_get_notification_by_id(
    ...
    response = await auth_client.get(f"/api/v1/notifications/{notification_id}")
    assert response.status_code == 200
    response_data = response.json()
    ...
    response_not_found = await auth_client.get("/api/v1/notifications/99999")
...
async def test_delete_notification(
    ...
    response = await auth_client.delete(f"/api/v1/notifications/{notification_id}")
    assert response.status_code == 204

    deleted_notification = await db_session.get(Notification, notification_id)
    assert deleted_notification is None

    response_not_found = await auth_client.delete("/api/v1/notifications/99999")
    assert response_not_found.status_code == 404
    assert "detail" in response_not_found.json()
...
async def test_get_notification_count(
    ...
    response_initial = await auth_client.get("/api/v1/notifications/count")
    ...
    response_after_create = await auth_client.get("/api/v1/notifications/count")
...
async def test_check_notification_exists(
    ...
    response_not_exists = await auth_client.get("/api/v1/notifications/99999/exists")
