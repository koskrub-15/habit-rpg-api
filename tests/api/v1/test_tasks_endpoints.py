...
async def test_get_task_by_id(
    ...
    response_not_found = await client.get("/api/v1/tasks/99999")
...
async def test_delete_task(
    ...
    response_not_found = await client.delete("/api/v1/tasks/99999")
    assert response_not_found.status_code == 404
    assert "detail" in response_not_found.json()
...
async def test_get_task_count(
    ...
    response_initial = await client.get("/api/v1/tasks/count")
...
async def test_check_task_exists(
    ...
    response_exists = await client.get(f"/api/v1/tasks/{task_id}/exists")
    assert response_exists.status_code == 200
    assert response_exists.json()["exists"] is True

    response_not_exists = await client.get("/api/v1/tasks/99999/exists")
...
async def test_bulk_create_tasks(
    ...
    response = await client.post("/api/v1/tasks/bulk", json=tasks_payload)
    assert response.status_code == 201
    response_data = response.json()
    assert isinstance(response_data, list)
    assert len(response_data) == 2
