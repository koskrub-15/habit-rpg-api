...
@pytest.mark.asyncio
async def test_grant_achievement_to_user(client: AsyncClient, db_session: AsyncSession):
    """
    Tests manual granting of an achievement to a user.
    """
    user = User(name="Achiever", email="achiever@example.com", password="password")
    achievement = Achievement(name="Early Bird", condition_type="tasks", condition_value=1, description="First task")
    db_session.add_all([user, achievement])
    await db_session.commit()
    
    response = await client.post(f"/api/v1/users/{user.id}/achievements/{achievement.id}")
...
@pytest.mark.asyncio
async def test_grant_nonexistent_achievement_returns_404(client: AsyncClient, db_session: AsyncSession):
    """
    ...
    response = await client.post(f"/api/v1/users/{user.id}/achievements/9999")
    assert response.status_code == 404

@pytest.mark.asyncio
async def test_grant_duplicate_achievement_returns_409(client: AsyncClient, db_session: AsyncSession):
    """
    ...
    achievement = Achievement(name="One Time", condition_type="tasks", condition_value=1, description="Only once")
    db_session.add_all([user, achievement])
    await db_session.commit()
    
    # Grant first time
    await client.post(f"/api/v1/users/{user.id}/achievements/{achievement.id}")
...
@pytest.mark.asyncio
async def test_complete_task_triggers_achievement_check(client: AsyncClient, db_session: AsyncSession):
    """
    ...
    achievement = Achievement(name="Task Master", condition_type="tasks_completed", condition_value=1, description="Completed 1 task")
    ...
    response = await client.patch(f"/api/v1/tasks/{task.id}", json={"status": "COMPLETED"})
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_complete_habit_streak_triggers_achievement(client: AsyncClient, db_session: AsyncSession):
    """
    ...
    achievement = Achievement(name="Consistent", condition_type="habit_streak", condition_value=5, description="Streak of 5")
    habit = Habit(name="Gym", user=user, streak=4, habit_type=HabitType.POSITIVE, status=HabitStatus.TODO)
    ...
    response = await client.post(f"/api/v1/habits/{habit.id}/complete")
    assert response.status_code == 200
