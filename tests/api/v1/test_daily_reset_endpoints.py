...
@pytest.mark.asyncio
async def test_reset_daily_tasks_also_resets_habits(client: AsyncClient, db_session: AsyncSession):
    """
    ...
    task = Task(name="Daily Task", user_id=user.id, task_type=TaskType.DAILY, status=TaskStatus.COMPLETED)
    # Create a completed positive habit
    habit = Habit(name="Daily Habit", user_id=user.id, habit_type=HabitType.POSITIVE, status=HabitStatus.COMPLETED)
