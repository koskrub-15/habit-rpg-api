...
async def test_get_shop_rotation_items(
    ...
    response = await auth_client.get("/api/v1/shop_rotation_items/")
...
async def test_get_shop_rotation_items_filter_by_name(
    ...
    response = await auth_client.get("/api/v1/shop_rotation_items/?name=Entry 1")
    assert response.status_code == 200
    response_data = response.json()
...
async def test_get_shop_rotation_item_by_id(
    ...
    response = await auth_client.get(f"/api/v1/shop_rotation_items/{sri_id}")
    assert response.status_code == 200
    response_data = response.json()
    ...
    response_not_found = await auth_client.get("/api/v1/shop_rotation_items/99999")
...
async def test_delete_shop_rotation_item(
    ...
    response = await auth_client.delete(f"/api/v1/shop_rotation_items/{sri_id}")
    assert response.status_code == 204

    deleted_sri = await db_session.get(ShopRotationItem, sri_id)
    assert deleted_sri is None

    response_not_found = await auth_client.delete("/api/v1/shop_rotation_items/99999")
    assert response_not_found.status_code == 404
    assert "detail" in response_not_found.json()
...
async def test_get_shop_rotation_item_count(
    ...
    response_initial = await auth_client.get("/api/v1/shop_rotation_items/count")
    ...
    response_after_create = await auth_client.get("/api/v1/shop_rotation_items/count")
