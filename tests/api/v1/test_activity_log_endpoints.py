...
@pytest.mark.asyncio
async def test_complete_task_creates_log_entry(client: AsyncClient, db_session: AsyncSession):
    """
    ...
    # Check logs
    response = await client.get(f"/api/v1/activity-log/", params={"user_id": user.id})
...
@pytest.mark.asyncio
async def test_complete_habit_creates_log_entry(client: AsyncClient, db_session: AsyncSession):
    """
    ...
    habit = Habit(name="Log Habit", user=user, habit_type=HabitType.POSITIVE, status=HabitStatus.TODO)
    ...
    assert response.status_code == 200
    logs = response.json()
    assert any(log["activity_type"] == "HABIT_COMPLETED" and log["habit_id"] == habit.id for log in logs)
...
async def test_activity_log_pagination(client: AsyncClient, db_session: AsyncSession):
    ...
    # Test pagination
    response = await client.get(f"/api/v1/activity-log/", params={"user_id": user.id, "skip": 5, "limit": 5})
    assert response.status_code == 200
    logs = response.json()
    assert len(logs) == 5
