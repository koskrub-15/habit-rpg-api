...
async def test_get_achievements_filter_by_name(
    ...
    response = await auth_client.get("/api/v1/achievements/?name=Login")
    assert response.status_code == 200
    response_data = response.json()
...
async def test_get_achievement_by_id(
    ...
    response = await auth_client.get(f"/api/v1/achievements/{achievement_id}")
    assert response.status_code == 200
    response_data = response.json()
    ...
    response_not_found = await auth_client.get("/api/v1/achievements/99999")
...
async def test_delete_achievement(
    ...
    response = await auth_client.delete(f"/api/v1/achievements/{achievement_id}")
    assert response.status_code == 204

    deleted_achievement = await db_session.get(Achievement, achievement_id)
    assert deleted_achievement is None

    response_not_found = await auth_client.delete("/api/v1/achievements/99999")
    assert response_not_found.status_code == 404
    assert "detail" in response_not_found.json()
...
async def test_get_achievement_count(
    ...
    response_initial = await auth_client.get("/api/v1/achievements/count")
    ...
    response_after_create = await auth_client.get("/api/v1/achievements/count")
...
async def test_check_achievement_exists(
    ...
    response_not_exists = await auth_client.get("/api/v1/achievements/99999/exists")
