...
async def test_get_rewards(
    ...
    response = await auth_client.get("/api/v1/rewards/")
...
async def test_get_rewards_filter_by_name(
    ...
    response = await auth_client.get("/api/v1/rewards/?name=Bonus")
    assert response.status_code == 200
    response_data = response.json()
...
async def test_get_reward_by_id(
    ...
    response = await auth_client.get(f"/api/v1/rewards/{reward_id}")
    assert response.status_code == 200
    response_data = response.json()
    ...
    response_not_found = await auth_client.get("/api/v1/rewards/99999")
...
async def test_delete_reward(
    ...
    response = await auth_client.delete(f"/api/v1/rewards/{reward_id}")
    assert response.status_code == 204

    deleted_reward = await db_session.get(Reward, reward_id)
    assert deleted_reward is None

    response_not_found = await auth_client.delete("/api/v1/rewards/99999")
    assert response_not_found.status_code == 404
    assert "detail" in response_not_found.json()
...
async def test_get_reward_count(
    ...
    response_initial = await auth_client.get("/api/v1/rewards/count")
    ...
    response_after_create = await auth_client.get("/api/v1/rewards/count")
...
async def test_check_reward_exists(
    ...
    response_exists = await auth_client.get(f"/api/v1/rewards/{reward_id}/exists")
    assert response_exists.status_code == 200
    assert response_exists.json()["exists"] is True

    response_not_exists = await auth_client.get("/api/v1/rewards/99999/exists")
...
async def test_bulk_create_rewards(
    ...
    response = await auth_client.post("/api/v1/rewards/bulk", json=rewards_payload)
    assert response.status_code == 201
    response_data = response.json()
    assert isinstance(response_data, list)
    assert len(response_data) == 2
