...
async def test_buy_item_adds_to_inventory(
    ...
    """
    user = User(
        name="Buyer2", email="buyer2@example.com", password="password", gold=100
    )
...
async def test_buy_item_not_enough_gold_returns_400(
    ...
    """
    user = User(
        name="Poor User", email="poor@example.com", password="password", gold=10
    )
    item = Item(
        name="Expensive Item", item_type=ItemType.MISC, description="Rich stuff"
    )
    db_session.add_all([user, item])
    await db_session.flush()

    shop_item = ShopItem(
        name="Expensive Item in Shop", item_id=item.id, price=100, stock=1
    )
    db_session.add(shop_item)
    await db_session.commit()
...
async def test_buy_out_of_stock_returns_400(
    ...
    """
    user = User(
        name="Buyer3", email="buyer3@example.com", password="password", gold=100
    )
...
async def test_buy_reduces_stock_by_one(client: AsyncClient, db_session: AsyncSession):
    ...
    """
    user = User(
        name="Buyer4", email="buyer4@example.com", password="password", gold=100
    )
    item = Item(
        name="Limited Item", item_type=ItemType.MISC, description="Limited edition"
    )
    db_session.add_all([user, item])
    await db_session.flush()

    shop_item = ShopItem(
        name="Limited Item in Shop", item_id=item.id, price=10, stock=10
    )
    db_session.add(shop_item)
    await db_session.commit()
...
async def test_buy_item_outside_availability_window_returns_400(
    ...
    """
    user = User(
        name="Buyer5", email="buyer5@example.com", password="password", gold=100
    )
