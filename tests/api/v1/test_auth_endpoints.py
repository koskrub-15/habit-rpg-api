...
@pytest.mark.asyncio
async def test_login_nonexistent_user_returns_401(client: AsyncClient, db_session: AsyncSession):
    """
...
async def test_refresh_token_returns_new_access_token(client: AsyncClient, db_session: AsyncSession):
    ...
    data = response.json()
    assert "access_token" in data
    assert "refresh_token" in data  # Usually rotation happens

@pytest.mark.asyncio
async def test_refresh_with_invalid_token_returns_401(client: AsyncClient, db_session: AsyncSession):
    """
