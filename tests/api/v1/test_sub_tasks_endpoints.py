...
async def test_get_sub_tasks(
    ...
    response = await auth_client.get("/api/v1/sub_tasks/")
...
async def test_get_sub_tasks_filter_by_name(
    ...
    response = await auth_client.get("/api/v1/sub_tasks/?name=Subtask 1")
    assert response.status_code == 200
    response_data = response.json()
...
async def test_get_sub_task_by_id(
    ...
    response = await auth_client.get(f"/api/v1/sub_tasks/{sub_task_id}")
    assert response.status_code == 200
    response_data = response.json()
    ...
    response_not_found = await auth_client.get("/api/v1/sub_tasks/99999")
...
async def test_delete_sub_task(
    ...
    response = await auth_client.delete(f"/api/v1/sub_tasks/{sub_task_id}")
    assert response.status_code == 204

    deleted_sub_task = await db_session.get(SubTask, sub_task_id)
    assert deleted_sub_task is None

    response_not_found = await auth_client.delete("/api/v1/sub_tasks/99999")
    assert response_not_found.status_code == 404
    assert "detail" in response_not_found.json()
...
async def test_get_sub_task_count(
    ...
    response_initial = await auth_client.get("/api/v1/sub_tasks/count")
    ...
    response_after_create = await auth_client.get("/api/v1/sub_tasks/count")
...
async def test_check_sub_task_exists(
    ...
    response_exists = await auth_client.get(f"/api/v1/sub_tasks/{sub_task_id}/exists")
    assert response_exists.status_code == 200
    assert response_exists.json()["exists"] is True

    response_not_exists = await auth_client.get("/api/v1/sub_tasks/99999/exists")
...
async def test_bulk_create_sub_tasks(
    ...
    response = await auth_client.post("/api/v1/sub_tasks/bulk", json=sub_tasks_payload)
    assert response.status_code == 201
    response_data = response.json()
    assert isinstance(response_data, list)
    assert len(response_data) == 2
