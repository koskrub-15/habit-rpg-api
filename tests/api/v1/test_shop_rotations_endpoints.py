...
async def test_get_shop_rotation_by_id(
    ...
    response = await client.get(f"/api/v1/shop_rotations/{rotation_id}")
    assert response.status_code == 200
    response_data = response.json()
...
async def test_delete_shop_rotation(
    ...
    response = await client.delete(f"/api/v1/shop_rotations/{rotation_id}")
    assert response.status_code == 204

    deleted_rotation = await db_session.get(ShopRotation, rotation_id)
    assert deleted_rotation is None

    response_not_found = await client.delete("/api/v1/shop_rotations/99999")
    assert response_not_found.status_code == 404
    assert "detail" in response_not_found.json()
...
async def test_get_shop_rotation_count(
    ...
    response_initial = await client.get("/api/v1/shop_rotations/count")
...
async def test_check_shop_rotation_exists(
    ...
    response_exists = await client.get(f"/api/v1/shop_rotations/{rotation_id}/exists")
    assert response_exists.status_code == 200
    assert response_exists.json()["exists"] is True

    response_not_exists = await client.get("/api/v1/shop_rotations/99999/exists")
